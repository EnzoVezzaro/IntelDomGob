# IntelDomGob Studio Setup Guide

This page keeps the detailed install, deployment, troubleshooting, and configuration notes out of the front README.

## Quick Start

> **Branch note:** `dev` is the default branch and contains the latest development changes, but it may be unstable. For the more stable curated branch, use [`main`](https://github.com/EnzoVezzaro/IntelDomGob/tree/main).

Defaults work out of the box: clone, run, then configure models/search/email
inside **Settings**. Only edit `.env` for deployment-level overrides like
`APP_BIND`, `APP_PORT`, `AUTH_ENABLED`, `DATABASE_URL`, or a pre-seeded admin password.

On first setup, IntelDomGob Studio creates an admin account (`admin` unless
...
M-series Mac, run IntelDomGob Studio natively:
...
**Docker bundled services.** Compose starts IntelDomGob Studio, ChromaDB, SearXNG, and
ntfy. IntelDomGob Studio and the bundled service ports bind to `127.0.0.1` by default, so
...
IntelDomGob Studio SSH key and add the public key to the remote server's
...
does not mount `/var/run/docker.sock`. You can still connect IntelDomGob Studio to
...
On Arch Linux, verify the host NVIDIA driver and Docker GPU passthrough before enabling the IntelDomGob Studio NVIDIA overlay.
...
Then enable the IntelDomGob Studio NVIDIA compose overlay:

```env
COMPOSE_FILE=docker-compose.yml:docker/gpu.nvidia.yml
```

Rebuild and verify the GPU inside the IntelDomGob Studio container:
...
> inside the slim IntelDomGob Studio image.
...
This connects IntelDomGob Studio in Docker to an Ollama server that is already running on
your host machine; it does not start Ollama inside the container.
`host.docker.internal` is Docker's hostname for the host machine from inside the
container. Cookbook **Serve** is a separate workflow for serving downloaded
models through IntelDomGob Studio/llama.cpp, so Windows users with an existing Ollama
...
do not run on macOS. MLX-only models are not served by IntelDomGob Studio.
...
[Ollama](https://ollama.com/download) is the easiest path — point IntelDomGob Studio at
...
If `chromadb-client` (the lightweight HTTP-only package) is installed alongside the full `chromadb` package, IntelDomGob Studio starts but ChromaDB silently falls back to HTTP-only mode and fails.
...
To expose IntelDomGob Studio on a local network or Tailscale with HTTPS:
...
3. Run `uvicorn` with the generated certs:
   ...
   ```
4. Install the `mkcert` CA on any other device you want to access IntelDomGob Studio from (e.g., for iOS, email the `rootCA.pem` to yourself, install the profile, and trust it in Certificate Trust Settings).

### Common self-host traps (30-second fixes)
A grab-bag of small gotchas that otherwise turn into long debugging sessions.

- **`AUTH_ENABLED=false` is ignored / you're still forced to log in (Windows).** If you edited `.env` in Notepad it may have saved a UTF-8 **BOM**, turning the first key into `﻿AUTH_ENABLED` so it is never matched. IntelDomGob Studio loads `.env` with `encoding="utf-8-sig"` to tolerate a leading BOM, but the safe fix is to re-save `.env` as **UTF-8 without BOM** (VS Code: *Save with Encoding → UTF-8*).
...
- **Calendar/contacts (Radicale) won't sync.** Point IntelDomGob Studio at the **full collection URL** with its trailing slash — e.g. `http://host:5232/<user>/<collection-id>/` — not just the server root. Radicale shows this address for each calendar/address book in its web UI.
...
IntelDomGob Studio email accounts currently use IMAP/SMTP username-password auth. Outlook
and Microsoft 365 generally require OAuth instead, so normal Microsoft mailbox
passwords will fail. See [docs/email-outlook.md](docs/email-outlook.md) for the
current limitation and the planned integration direction.

## Security Notes
IntelDomGob Studio is a self-hosted workspace with powerful local tools: shell access, file uploads, model downloads, web research, email/calendar integrations, and API tokens. Treat it like an admin console.

- Keep `AUTH_ENABLED=true` for any network-accessible deployment.
- Keep `LOCALHOST_BYPASS=false` outside local development.
- Use `SECURE_COOKIES=true` when IntelDomGob Studio is served through HTTPS by a trusted reverse proxy or private access gateway.
...
- Keep ChromaDB, SearXNG, ntfy, Ollama, vLLM, llama.cpp, databases, and raw model/provider APIs internal-only. Expose only the authenticated IntelDomGob Studio web/API entrypoint through your trusted proxy or private access layer.
- Before publishing a fork, run `git status --short` and confirm no private files from `.env`, `data/`, `logs/`, uploads, backups, or local databases are staged.

### Private or proxied deployments
IntelDomGob Studio serves plain HTTP on its app port. Docker Compose binds IntelDomGob Studio and the bundled services to `127.0.0.1` by default, so a typical production/private setup is:

1. Keep IntelDomGob Studio on localhost, for example `127.0.0.1:7000`.
2. Terminate HTTPS at a trusted reverse proxy or private access gateway.
3. Put the authenticated IntelDomGob Studio web/API entrypoint behind that layer.
4. Keep raw service and model ports internal-only.

Cloudflare Access, Tailscale, Caddy, nginx, and Traefik can all fit this pattern; none are required by IntelDomGob Studio. If your access layer reaches IntelDomGob Studio on the same host, proxy to `http://127.0.0.1:7000` and keep `AUTH_ENABLED=true`, `LOCALHOST_BYPASS=false`, and `SECURE_COOKIES=true`.
...
| Port | Service |
|---|---|
| `7000` | IntelDomGob Studio raw app port |
| `8080` | SearXNG |
| `8091` | ntfy |
...
| `SECURE_COOKIES` | `false` | Set true when serving IntelDomGob Studio through HTTPS at a trusted proxy or private access gateway. |
...
IntelDomGob Studio auto-registers a few built-in MCP servers at startup. The npx-based ones (currently the browser server, `@playwright/mcp`) only start when their npm package is already in the local npx cache. If a package isn't cached, that server is skipped with a startup log message explaining what to do, so a fresh install does not block on a multi-minute npm download or hang if Playwright system deps are missing.
...
That installs `@playwright/mcp` plus Playwright (~300MB total). Restart IntelDomGob Studio and the server will register at startup.
