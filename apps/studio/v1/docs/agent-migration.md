...
IntelDomGob Studio should be able to learn from another agent without blindly trusting
...
Markdown notes, or any other agent can have its own adapter, but IntelDomGob Studio only
...
- **Memory candidates** are short facts or preferences that can be reviewed
  before being   saved into IntelDomGob Studio memory.

This keeps IntelDomGob Studio' existing memory-review flow intact while giving it better
...
The helper does not write to `data/`, call an LLM, import IntelDomGob Studio modules, or
...
A future IntelDomGob Studio importer should treat the manifest as untrusted user-provided
...
Nonstandard folders should be adapter details, not required IntelDomGob Studio concepts.
