...
class TaskScheduler:
        ...
        try:
            ...
            if gate_foreground:
                ...
                if waiting and waiting.status == "queued":
                    waiting.result = f"Queued — waiting for {BRAND_NAME} to be idle…"
                    db.commit()
                ...
                async def _cancel_if_foreground_active():
                    ...
                    while True:
                        ...
                        if has_foreground_activity():
                            ...
                            logger.info("Task '%s' interrupted because %s became active", task.name, BRAND_NAME)
                            if current_task:
                                current_task.cancel()
                            return
            ...
            try:
                ...
                msg = (
                    f"Paused because {BRAND_NAME} became active"
    ...
    async def stop_background_tasks_for_foreground(self, *, reason: str = f"{BRAND_NAME} became active") -> int:
        ...
        user opens or uses the app, foreground interaction wins immediately.
