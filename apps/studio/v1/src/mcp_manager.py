...
class McpManager:
    ...
    def get_all_tools(self, disabled_map: Optional[Dict[str, set]] = None) -> List[Dict]:
        ...
        for server_id, tools in self._tools.items():
            ...
            disabled = (disabled_map or {}).get(server_id, set())
            for tool in tools:
                # Human-readable title from MCP annotations (2025-03-26), if the
                # server provided one (e.g. "Consulta de Inteligencia"). Used by
                # the UI to label tool-call bubbles instead of the raw qualified
                # name. annotations may be a pydantic model or a plain dict.
                _ann = tool.get("annotations")
                _title = ""
                if _ann is not None:
                    _title = (
                        getattr(_ann, "title", None)
