    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if _force_answer:
            ...
            if _relevant_tools:
                ...
                # MCP tools are NOT filtered by the RAG _relevant_tools set.
                # The RAG tool index is keyed on built-in tool names and often
                # fails to surface dynamically-connected MCP tools (especially
                # for non-English queries or when the index wasn't populated with
                # the live MCP manager). Meanwhile their descriptions are always
                # advertised to the model via get_tool_descriptions_for_prompt(),
                # so filtering them out here made the model call tools it was told
                # it had — which bounced as "Unknown action". Connected MCP tools
                # are already gated by the enabled-server check and the per-tool
                # disabled_map, so include them all. (See regression: MCP tools
                # missing from tools_sent while server showed "connected".)
