...
# Branding
# Single source of truth for the public-facing product name. Override via the
# APP_NAME env var (see .env.example) so the display name can be customized per
# deployment without editing source. Internal identifiers (env var names,
# HTTP headers, DB collections, data paths) intentionally keep their original
# names for backward compatibility and clean upstream merges.
BRAND_NAME = os.getenv("APP_NAME", "IntelDomGob Studio")
