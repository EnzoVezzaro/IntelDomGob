...
// Display label for a tool name. For MCP tools (mcp__<server_id>__<tool>),
// prefer the human-readable annotation title (shared via window._mcpToolTitles,
// populated by chat.js), then fall back to a prettified bare tool name.
function _prettyMcpToolName(rawName) {
  const name = String(rawName || 'tool');
  if (!name.startsWith('mcp__')) return name;
  const titles = (typeof window !== 'undefined' && window._mcpToolTitles) || {};
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'IntelDomGob Studio';
...
export function addMessage(role, content, modelName, metadata) {
  try {
      ...
      for (let r = 0; r < maxRound; r++) {
        ...
        if (roundTools.length > 0) {
          ...
          for (const ev of roundTools) {
            ...
            node.innerHTML = `<div class="agent-thread-dot"></div><div class="agent-thread-header"><span class="agent-thread-icon">${ok ? '\u2713' : '\u2717'}</span><span class="agent-thread-tool">${esc(_prettyMcpToolName(ev.tool))}</span><span class="agent-thread-status">${ok ? 'done' : 'failed'}</span><span class="agent-thread-chevron">\u25B6</span></div><div class="agent-thread-content">${evCmdHtml}${outHtml}${evDiffHtml}</div>`;
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'IntelDomGob Studio' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
