...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'IntelDomGob Studio Chat';
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to IntelDomGob Studio kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'IntelDomGob Studio Chat';
    }
