...
export const ERROR_PATTERNS = [
  ...
  },
  {
    ...
    fixes: [
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('vllm') },
      {
        ...
        action: () => _copyText('Upgrade the vLLM environment that provides the selected vllm CLI, or use a compatible checkpoint. Do not assume IntelDomGob Studio owns PATH/system/source/Docker installs.'),
      },
    ],
  },
  {
  ...
  },
  {
    ...
    suggestion: 'Suggested action: relaunch with IntelDomGob Studio’ venv CUDA library path fix. If the venv does not contain the matching NVIDIA runtime libs, run Repair sglang-kernel.',
    fixes: [
  ...
  },
  {
    ...
    suggestion: 'Suggested action: relaunch from the cached local snapshot path. IntelDomGob Studio now rewrites MLX repo-id launches to the newest local Hugging Face snapshot when it exists on the selected Mac.',
    fixes: [
      { label: 'Edit / relaunch serve', action: (panel) => _openServeEditFromDiagnosis(panel) },
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('mlx_lm') },
      { label: 'Copy error', action: (panel) => {
...
function _diagnosisCopyBundle(task, diagnosis, sourceText, suggestionText) {
  const lines = ['## IntelDomGob Studio Cookbook troubleshooting'];
  if (task) {
    lines.push(
      '',
      '### Task',
