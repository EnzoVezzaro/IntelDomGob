#!/bin/bash
# IntelDomGob Studio — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything IntelDomGob Studio needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine IntelDomGob Studio
...
echo "▶ IntelDomGob Studio quick start for macOS"
...
echo "▶ Preparing IntelDomGob Studio…"
    ...
    (
        for _ in $(seq 1 90); do
            if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
                printf '\n'
                printf '  ┌──────────────────────────────────────────────────────┐\n'
                printf '  │  ✓ IntelDomGob Studio is ready — opening your browser  │\n'
                printf '  │     %-50s │\n' "$URL"
...
echo
echo "▶ Starting IntelDomGob Studio — it will open in your browser at $URL"
