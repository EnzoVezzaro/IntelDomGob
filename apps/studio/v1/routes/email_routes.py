...
from src.constants import BRAND_NAME, DATA_DIR
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the marker header, but include subject fallbacks too.
                # The fallbacks use distinct branded prefixes so ordinary emails
                # containing "Reminder" don't get mixed in. Both the current
                # brand prefix and the legacy "Reminder (Odysseus):" prefix are
                # matched so reminders sent before a rebrand still show up.
                # IMAP OR is binary, so multiple alternatives are nested:
                # OR a (OR b c)  ->  "OR a OR b c".
                _brand_subject = f"Reminder ({BRAND_NAME}):"
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-Odysseus-Kind "reminder" OR SUBJECT "{_brand_subject}" SUBJECT "Reminder (Odysseus):"{from_clause})',
                )
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        # Subject fallbacks: the current brand prefix plus the
                        # legacy "Reminder (Odysseus):" prefix so reminders sent
                        # before a rebrand are still swept.
                        _subject_prefixes = {f"Reminder ({BRAND_NAME}):", "Reminder (Odysseus):"}
                        for _pfx in _subject_prefixes:
                            uids.update(_search_uids(conn, f'(SUBJECT {_search_quote(_pfx)})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            for _pfx in _subject_prefixes:
                                uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote(_pfx)})'))
                            # Legacy reminders created before the branded prefix
                            # still came from this mailbox as
